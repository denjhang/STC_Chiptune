/* saa1099.c - SAA1099 仿真核心 (精简嵌入式版)
 * 6 方波 + 2 噪声 + 2 包络, 18-bit LFSR
 * 参考 libvgm saa1099_vb.c (Valley Bell)
 * 精简: 单声道混音
 */
#include <stc8h.h>
#include "saa1099.h"
...
static u8 code saa_env_tbl[8][32] = {
...
static u8 code saa_voltbl[16] = {
      0,  8, 17, 25, 34, 42, 51, 59,
     68, 76, 85, 93, 102, 110, 119, 127
};

/* 通道 */
static u8  xdata saa_vol[6];
static u8  xdata saa_freq[6];
static u8  xdata saa_oct[6];
static u8  xdata saa_tone_on;
static u8  xdata saa_noise_on;
static u8  xdata saa_state[6];
static s16 xdata saa_fcount[6];
static s16 xdata saa_flimit[6];

/* 噪声 */
static u32 xdata saa_nstate[2];
static u8  xdata saa_nmode[2];
static s16 xdata saa_ncnt[2];
static s16 xdata saa_nlimit[2];

/* 包络 */
static u8  xdata saa_env_en[2];
static u8  xdata saa_env_rld[2];
static u8  xdata saa_env_ext[2];
static u8  xdata saa_env_step2[2];
static u8  xdata saa_env_wave[2];
static u8  xdata saa_env_pos[2];
static u8  xdata saa_env_flags[2];
static u8  xdata saa_env_vol[2];

static u8  xdata saa_all_on;
static u8  xdata saa_fg_rst;
static u8  xdata saa_addr;
static u8  xdata saa_regs[0x20];
static u32 xdata saa_base_cnt;
