 ...
 * 支持三种变体: SN76489(15bit), SegaVDP(16bit), SN76489A(17bit)
 * 参考 libvgm sn76489.c (Maxim)
 */
#include <stc8h.h>
#include "sn76489.h"

#define SN_GETA_BITS 24
/* base_incr = CLK * (1<<24) / 16 / SAMPLE_RATE
 * = 3579545 * 16777216 / 16 / 17640 = 212779193 */
...
static u32 xdata sn_rng;
static u32 xdata sn_base_count;

/* 变体参数 */
static u8  xdata sn_sr_width;
static u16 xdata sn_taps;
static u32 xdata sn_rng_init;
...
s16 sn_render(void) {
    ...
    if (incr > 0) {
        ...
        if (sn_period[3] > 0 && sn_count[3] >= sn_period[3]) {
            u8 fb;
            if (sn_reg[6] & 0x04) {
                /* 白噪音: 两bit taps parity (libvgm 优化) */
