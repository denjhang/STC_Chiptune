            ...
            # NES APU 路径: 0x90 绑定 $4011 写, 0x95 按 sound ID 播放
                ...
                if pos + 11 <= end:
                    ...
                    if bank is not None and db_pos < len(bank['data']):
                        ...
                        # 同步发送 (与 0x67 0xC2 同理: 保证数据在 $4015 trigger 前到位)
                        last_time = time.perf_counter()
                ...
                if pos + 6 <= end:
                    ...
                    if dmc_stream_mode and tp == 0xC2 and sz >= 2 and pos + 6 + sz <= end:
                        ...
                        # 同步发送整块 (保证 DMC 数据在 $4015 trigger 之前到位).
                        # 之前异步队列导致 $4015 在 0xB6 之前到达 → DMC 引擎读空 buffer → 卡无声.
                        # 时序安全: VGM 的 0xC2 LOAD 总在 $4015 TRIG 之前 (同一样本点),
                        #   所以这里同步发完再继续, $4015 一定在数据之后.
                        # 重置 last_time 让传输耗时不计入 samples_budget (避免追跑).
                        last_time = time.perf_counter()
