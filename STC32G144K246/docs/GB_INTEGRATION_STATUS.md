...
## 15. RC 高通滤波器尝试 (2026-06-22, 失败回退 — gbsplay 对比)

Bug 8 (square duty) 修复后, Pokemon New Bark Town 的 12.5% duty 通道仍有嗡嗡/破音.
用户直觉认为 libvgm 的 GB 核心音质不如 gbsplay, 指引对比 gbsplay 源码.

### gbsplay vs libvgm 对比发现

参考源: `Reference_Project/vgm_libs/modizer-master/libs/gbsplay/gbsplay/gbhw.c`

gbsplay 实现了 RC 高通滤波器 (gbhw.c line 684-696), 模拟 DMG 真机硬件:
CPU 输出经 **1μF 电容 + 510Ω + 10KΩ 电位器**到放大器, 截止 **15.14Hz**.
作用: 隔直 (消除 duty 不对称直流) + 衰减次声波.
**libvgm 没做这个高通** — 这是 gbsplay 音质比 libvgm 干净的一个原因.

### 实现尝试 (已回退)

加了标准一阶 RC 高通: `y[n] = α × (y[n-1] + x[n] - x[n-1])`, α=0.9957 (Q16=65254).
关键坑: 信号放大到 Q16 再滤波, 否则小信号整数截断导致衰减失效.
仿真验证 12.5% duty 直流 -10.6 → -0.47 (消除 96%).

### 实机结果: 音质更差, 已回退

烧录测试 Pokemon, **音质反而更差**. 已撤销 RC 高通回到 Bug 8 修复后的状态 (47937 bytes).

### 失败原因分析 (重要教训)

仿真数学正确 (直流消除 96%), 实机却更差, 说明:

1. **α=0.996 衰减太慢** (τ=10ms, 5τ=50ms 暂态). 每个音符触发后, 高通要 50ms 才收敛,
   这个暂态过程本身产生低频波动, 可能比直流本身更难听 (类似 "扑扑" 声).

2. **15Hz 截止可能伤及音乐低频**. Pokemon 的贝斯线、鼓点有 <30Hz 的次声成分,
   高通把它们衰减了, 听感变薄.

3. **gbsplay 的好音质不只来自 RC 高通**. 它还用**冲激响应积分器** (impulse buffer,
   band-limited synthesis 抗混叠). RC 高通是配合冲激响应工作的 — 冲激响应把 GB cycle
   级信号正确折叠到采样率, RC 高通再隔直. 单独搬高通, 我们的 phaseacc 降采样方式和它
   不匹配, 反而引入暂态失真.

### 教训 (比成功更值钱)

1. **移植不能只搬一个组件**. 参考实现的各个部分是协同设计的 (impulse buffer + RC 高通),
   要么整套搬, 要么不搬. 单独搬一个组件可能破坏平衡.

2. **仿真数学正确 ≠ 实机音质改善**. 仿真只验证 "直流消除", 但听感涉及暂态、相位、
   频率响应曲线等多个维度. 数学上 "更对" 的滤波器可能听感 "更差".

3. **IIR 高通的暂态是隐形杀手**. α 接近 1 (慢衰减) 时暂态很长 (5τ=50ms),
   每次音符触发都会经历暂态, 听感是不稳定的波动. 这种问题仿真难发现, 实机一听就知道.

4. **用户直觉指方向, 但实现要谨慎**. "gbsplay 音质好" 是事实, 但 "搬它的 RC 高通就能改善"
   是错的推理. gbsplay 的优势是整体架构, 不是单个滤波器.

5. **12.5% duty 的嗡嗡是 DMG 原声**. 真机也有这个特性 (duty 不对称的数学必然).
   Pokemon CH1 用 12.5% duty 做低音线, 嗡嗡感是作曲家的音色选择, 不是 bug.
   接受它是 "原汁原味" 的正确态度.

### 当前状态

RC 高通已撤销, GB 回到 Bug 8 (square duty) 修复后的状态 (47937 bytes).
12.5% duty 的嗡嗡作为 DMG 已知特性保留, 不再尝试软件消除.

### 备选方案 (如果未来还想改善)

- **整套搬 gbsplay 的 impulse buffer + RC 高通** (工作量大, 要重写 render 架构)
- **硬件方案**: DAC 输出端加合适的隔直电容 + 高通网络 (硬件改板)
- **接受现状**: DMG 原声就是这样, 12.5% duty 本就有独特音色

## 16. NES Deflemask DAC stream (0x90-0x95) 支持 (2026-06-22)

### 背景

`vgm/nes/dek/` 目录的 Deflemask 导出 VGM (feeblemask, ygo10, ottifanten, pictonary)
完全不发声的 DMC 通道 — 鼓声/采样听不到. 用户报告 "另一窗口曾实现过完全正常的版本,
但丢失了上下文", 需要重新实现.
...
### 改动 (全在 vgm_player.py, 不动 nes.c/main.c)
...
DAC stream 已实现并 commit (`cc7631b`), PCM 鼓声清晰.
**遗留问题**: 播放非常不稳定 (节奏/卡顿), 待排查. 怀疑每字节 `ser.write` 的 Python
端开销让上位机 budget 算法失准, 导致节奏波动.

### 产物

- `STC32G144K246/usb_cdc_test/tools/vgm_player.py` (DAC stream 实现)
