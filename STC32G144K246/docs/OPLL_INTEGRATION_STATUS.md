...
### 3.5 鼓声方案 (PC 仿真验证中, 未改下位机)

鼓声 = 频率源 + 包络 + 可能的噪声. 噪声用 64 点假随机表 (±31), 不用 LFSR.
包络走完整 ADSR (和旋律一样的 AR/DR/SL/RR 查表).

| 鼓声 | 频率源 | 噪声 | ADSR (AR/DR/SL/RR) |
|------|--------|------|---------------------|
| BD | 纯正弦 100Hz | 无 | 15/8/6/13 |
| TOM | 正弦 214Hz (ml=5) | 无 | 15/8/5/9 |
| SD | — | 20%噪声混合 | 13/8/6/8 |
| HH | — | 纯噪声 755Hz | 12/8/10/7 |
| CYM | — | 纯噪声 755Hz | 10/10/5/5 |

- 噪声表: 64 点随机 ±31, 16.16 定点步进 (755Hz 查表速度)
- 噪声步进 `NOISE_STEP_Q16 = 755 × 64 × 65536 / INTERNAL_RATE`
- SD 波形 = 正弦 × 0.8 + 噪声 × 0.2 (noise_mix 控制)
- level=0 时强制静音 (避免 -1 泄漏)
- PC 工具: `tools/drum_fw_sim.py` (render_drum_fw)
