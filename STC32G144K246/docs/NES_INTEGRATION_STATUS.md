...
### 6.1 DMC 流式切 bank 卡顿: 三次尝试均失败 (2026-06-23, 最终回退同步)

**问题**: 流式模式 (>16KB) 遇 `0x67 0xC2` 时 `dmc_send_block` **同步发整块** (4-8KB),
阻塞 budget 循环 → 切 bank 时节奏短暂变慢.

**三次尝试解决 (全部失败, 已回退)**:

| 版本 | 方案 | 结果 | 失败原因 |
|------|------|------|----------|
| `40c1ac5` 异步队列 | 拆 chunks 推队列, budget 每轮发 512B | ❌ 卡无声 | $4015 TRIG 在 0xB6 数据之前到达 → DMC 读空 buffer |
| `b27eac4` 双缓冲 | 下位机 32KB buffer, 上位机管 page + 0xBA 切页 | ❌ 切块更卡 | 仍同步发送, 双缓冲并行优势没发挥 |
| `a087abb` 自动双缓冲 | 下位机自动写空闲页, $4015 TRIG 自动切页 | ❌ 性能跟不上 | 单核 MCU ISR + 主循环竞争, 异步发送速率不够 |

**最终回退到 `70d87c4` (同步发送)**:
- 遇 `0x67 0xC2` 时 `dmc_send_block` 同步发整块, 阻塞 budget
- 切 bank 时**节奏短暂变慢** (可接受, 不卡无声)
- 这是 USB CDC + 单核 MCU 架构的固有限制

**为什么 PCM ring 能异步而 DMC 不能**:
- PCM (0xB8) 是 2 字节定长命令, 丢字节后下一个 0xB8 能自动对齐
- DMC (0xB6) 是 4+len 变长命令, 丢一个字节就永久错位 → 后续全乱 → 卡无声
- PCM ring buffer 在下位机 (ISR 直接 pop), DMC 数据要写 nes_dmc_buf (主循环 process_uart)
- 时序约束不同: PCM 没有严格的 "数据在 trigger 前到位" 约束, DMC 有

**彻底解决需要**: 下位机自己解析 VGM (像 RPFM 那样 Core1 跑 VGM player),
上位机只灌原始字节. 但工作量远超当前收益, 暂不追求.
...
5. ❌ "DMC 切 bank 卡顿是 buffer 不够" — 错, 所有地址在 12KB 窗口内
6. ❌ "DMC 异步队列能解决切 bank 卡顿" — 错, $4015 TRIG 在数据前到达 → 卡无声
7. ❌ "DMC 双缓冲 (上位机管 page) 能并行" — 错, 仍同步发送, 并行优势没发挥
8. ❌ "DMC 双缓冲 (下位机自动切页) 能异步" — 错, 单核性能跟不上
...
6. **DMC 变长命令 (0xB6) 对丢字节零容忍**. 不像定长命令 (0xB8) 能自恢复.
   异步发送必须保证时序, 否则一条 0xB6 错位 → 全曲卡无声.

7. **双缓冲不是万能药**. 如果发送端仍同步阻塞, 双缓冲只是换了写的位置,
   没解决根本问题. 真正的并行需要发送端异步 + 接收端自动管理.
...
- ⚠️ **DMC 流式同步发送** (切 bank 短暂变慢, 三次异步/双缓冲尝试失败已回退)
  - 根因: USB CDC + 单核 MCU, DMC 变长命令对丢字节零容忍
  - 彻底解决需下位机自解析 VGM (RPFM 式), 暂不追求
